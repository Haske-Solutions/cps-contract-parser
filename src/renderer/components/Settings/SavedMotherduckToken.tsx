import type { MotherduckTokenPreview } from '@shared/types'
import { Badge } from '@/components/ui/badge'
import { Button } from '@/components/ui/button'
import { KeyRound, Trash2 } from 'lucide-react'

interface Props {
  preview: MotherduckTokenPreview
  onRemove?: () => void
}
...
export function SavedMotherduckToken({ preview, onRemove }: Props) {
  const sourceLabel =
    preview.source === 'keychain'
      ? 'Stored in OS keychain'
      : 'From MOTHERDUCK_TOKEN environment variable'

  return (
    <div
      className="rounded-lg border border-border bg-muted/20 overflow-hidden"
      role="listitem"
      aria-label="Saved MotherDuck token"
    >
      <div className="flex items-start justify-between gap-3 px-4 py-3 border-b border-border/60 bg-muted/30">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="text-sm font-medium truncate">MotherDuck access token</span>
        </div>
